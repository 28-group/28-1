...
st.markdown(
    ...
    .image-box img {
        ...
        position: relative !important;
        z-index: 10 !important;         /* 提高层级确保在图片框上方 */
    }
...
# 辅助函数：创建图片显示
def create_image_display(uploaded_file, default_text):
    """创建图片显示，确保在图片框内部"""
    if uploaded_file:
        # 如果已上传图片，打开并显示图片
        image = Image.open(uploaded_file)
        return st.image(image, use_container_width=False)
    else:
        # 如果未上传图片，显示占位内容
...
with col5:
    with st.container():
        ...
        if 'result_image' in st.session_state and st.session_state.result_image:
            # 显示结果图片
            st.image(st.session_state.result_image, use_container_width=False)
        else:
            # 如果未生成结果，显示占位内容
            st.markdown('''
            <div class="image-box-content">
