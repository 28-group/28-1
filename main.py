...
/* 隐藏"Drop files here"文本 */
.stFileUploader > div > div > div:nth-child(2) {
    display: none !important;
}
...
/* 隐藏文件名称显示 */
.stFileUploader > div > div > div[data-testid="stFileUploaderFileName"] {
    display: none !important;
}

/*
如何修改前两个上传图片的按钮，这边显示的是按钮包含一定的英文说明外加一个”browse files"的按键，英文说明大致意思是点击这里上传，我需要的是如何把按钮中的英文说明删掉，只保留”browse files"的按键
需要修改文件上传器的样式，隐藏默认的说明文字但保留"Browse files"按钮。以下是解决方案：
...
css
/* 文件上传器自定义样式 - 隐藏说明文字但保留按钮 */
.stFileUploader > div > div > div:first-child {
    display: none !important; /* 隐藏说明文字 */
}

.stFileUploader > div > div > div:nth-child(2) {
    display: none !important; /* 隐藏拖拽区域文字 */
}
